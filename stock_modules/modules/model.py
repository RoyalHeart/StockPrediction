...
class StockPrediction:
    ...
    def __init__(
        self,
        stock_code: StockCode = StockCode.SSI,
        start_date: datetime.datetime = datetime.datetime(2022, 1, 1),
        model_type: ModelType = ModelType.LSTM_DNN,
    ...
    def get_dataset(
        ...
        self.feature = dataset.shape[1]  # update feature number
        return dataset
    ...
    def create_trainXY_testXY(self):
        ...
        train_x, train_y = self.create_timestep_dataset(
            train_x, self.time_step, self.ahead, y_col=y_col
        )
        test_x, test_y = self.create_timestep_dataset(
            test_x, self.time_step, self.ahead, y_col=y_col
        )
    ...
    def reshape_input(self):
        train_x = np.reshape(
            self.train_x, (self.train_x.shape[0], self.time_step, self.feature)
        )
        test_x = np.reshape(
            self.test_x, (self.test_x.shape[0], self.time_step, self.feature)
        )
        self.train_x = train_x
        self.test_x = test_x
        ...
        if self.model_type == ModelType.CNN:
            model.add(
                Conv1D(
                    ...
                    (3),
                    activation="relu",
                    input_shape=(self.time_step, self.feature),
                )
            )
            ...
            model.add(LSTM(8, "relu", return_sequences=True))
            model.add(LSTM(4, "relu", input_shape=(self.time_step, 1)))
            model.add(Dense(4))
            # model.add(Dense(8))
            model.add(Dense(1))
        else:
    ...
    def evaluate_model(self):
        ...
        train_true = self.scaler.inverse_transform(self.train_y.reshape(-1, 1)).reshape(
            1, -1
        )
        test_predict = self.scaler.inverse_transform(test_predict)
        test_true = self.scaler.inverse_transform(self.test_y.reshape(-1, 1)).reshape(
            1, -1
        )
    ...
    def plot_result(self, dataset):
        ...
        train_predict_plot[
            self.time_step
            + self.ahead : len(self.train_predict)
            + self.time_step
            + self.ahead,
            :,
        ] = self.train_predict
        ...
        plt.clf()
        plt.plot(true_values[-len(self.test_predict) :])
        plt.plot(self.test_predict)
        plt.savefig(
            f"./{self.stock_dir}/{self.stock_code}/{self.version}/test_result.png"
        )
        plt.show()
        plt.close()
